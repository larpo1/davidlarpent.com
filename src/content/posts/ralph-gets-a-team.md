...
The PM breaks work into tasks, reviews completed output, and dictates what goes into the task file. The critical line is `disallowedTools: Write, Edit`. The PM cannot modify code. In the old two-terminal setup, the PM would drift into implementation within thirty seconds. Now it's locked out. 
